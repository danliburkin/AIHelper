...
describe('R2 snapshot envelope', () => {
  ...
  it('per-turn snapshots do not nest the full turn log (linear export growth)', async () => {
    const engine = createEngine();
    engine.setOriginalTask('Growth probe');

    const SMALL_REPLY = (n) => `===MEMORY===
- probe item ${n} | tags: growth
===END===`;

    const sizes = {};
    for (let i = 1; i <= 10; i++) {
      await engine.ingestReplyWithFallback(SMALL_REPLY(i));
      engine.addTurn(
        `Question turn ${i}?`,
        { memory: 1, facts: 0, assumptions: 0, ambient: 0 },
        0,
        'x'.repeat(200),
      );
      if (i === 5 || i === 10) {
        sizes[i] = JSON.stringify(engine.exportSnapshot()).length;
      }
    }

    expect(sizes[10]).toBeLessThan(3 * sizes[5]);
  });
});
